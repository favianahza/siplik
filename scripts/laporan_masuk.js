      ...
      new DataTable(sel, {
        ...
        columns: [
          ...
          { data: "alamat_lengkap" },
          {
            ...
            render: (ticket, type, row) => `
            <div class="d-flex flex-wrap justify-content-center">
                <a href="#detail" data-ticket="${encodeURIComponent(ticket)}" data-backload="laporan_masuk" class="btn btn-sm w-100 w-lg-0 mb-2 bg-primary text-white detail">DETAIL</a>
                <a href="#alokasi" data-ticket="${encodeURIComponent(ticket)}" class="btn btn-sm w-100 w-lg-0 ${row.status === "diverifikasi" ? "bg-warning text-white alokasi" : "bg-danger disabled"}">ALOKASI PETUGAS</a>
            </div>`
          },
        ],
    ...
    $main.html(`
      ...
      <section class="content py-3">
        <div class="container-fluid">
          ...
          <div class="text-end">
            <button class="btn btn-secondary" id="btn-back" onclick="loadComponent('${backLoad}')">← Kembali ke Daftar</button>
          ${verifyButton}
          </div>
        </div>
      </section>
